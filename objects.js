...
export function addBuildings(grid, gridSize, scene) {
  ...
  const buildingTexture = new THREE.TextureLoader().load(
    "Textures/building_night_texture.jpg"
  );
  ...
  for (let i = 0; i < gridSize; i++) {
    for (let j = 0; j < gridSize; j++) {
      ...
      if (grid[i][j] === 2 && !processed.has(key)) {
        ...
        const { minI, maxI, minJ, maxJ, count } = getClusterBounds(
          grid,
          i,
          j,
          processed,
        ...
        const distanceToCenter = Math.max(
          Math.abs(clusterCenterY - gridSize / 2),
          Math.abs(clusterCenterX - gridSize / 2)
        );
        const proximityScale =
          (gridSize / 2 - distanceToCenter) / (gridSize / 2); // Scaled 0-1, 1 is closest to center
        const randomHeight =
          1 + Math.random() * Math.sqrt(count) * proximityScale * 2; // Height based on cluster size and center proximity

        const cubeGeometry = new THREE.BoxGeometry(
          cellSize * clusterWidth,
          randomHeight,
          cellSize * clusterHeight
        );
        const building = new THREE.Mesh(cubeGeometry, buildingMaterial);
        building.position.set(
          (clusterCenterX - gridSize / 2) * cellSize,
          randomHeight / 2,
          (clusterCenterY - gridSize / 2) * cellSize
        );
        ...
        const outlineGeometry = new THREE.BoxGeometry(
          cellSize * clusterWidth * outlineScale,
          randomHeight * outlineScale,
          cellSize * clusterHeight * outlineScale
        );
...
export function addTrees(grid, gridSize, scene) {
  ...
  const treeGeometry = new THREE.CylinderGeometry(
    ...
    treeHeight,
    16
  ); // Create a cylinder to represent trees
  ...
  for (let i = 0; i < gridSize; i++) {
    for (let j = 0; j < gridSize; j++) {
      if (grid[i][j] === 4) {
        // Check if the cell type is for a tree
